...
export function detectVersionNumber(title: string): { found: boolean; version?: string; isDateVersion?: boolean; isStaleDateVersion?: boolean; dateValue?: Date; hasPreferredVersion?: boolean; preferredVersion?: string } {
  ...
  const regularVersionPatterns = [
    // Special patterns like v1.0a, v2.1b, v1.5-beta, v1.0alpha, v1.33.a, v1.2.3c, v0.4.1f5 (letter+digits)
    // This includes support for Unity-style versions like f5, f12, etc.
    /\bv(\d+(?:\.\d+)*(?:\.[a-z]\d*|[a-z]\d*)?(?:\-?(?:alpha|beta|rc|final|release|hotfix|patch))?)\b/i,
    ...
    /\bversion[\s\-\.]?(\d+(?:\.\d+){0,3}(?:\.[a-z]\d*|[a-z]\d*)?(?:\-?(?:alpha|beta|rc|final|release|hotfix|patch))?)\b/i,
    /\bver[\s\-\.]?(\d+(?:\.\d+){0,3}(?:\.[a-z]\d*|[a-z]\d*)?(?:\-?(?:alpha|beta|rc|final|release|hotfix|patch))?)\b/i,
